...
removeFixup() easier and more efficient.  Every RedBlackNodeString instantiated has
...
course textbook and so are the leftRotate(RedBlackNodeString x) and
rightRotate(RedBlackNodeString y) functions.
...
our delete(RedBlackNodeString v) function with a call to search(v.key) which will
...
leftRotateFixup(RedBlackNodeString) and rightRotateFixup(RedBlackNodeString) which update
these variables when a rotation occurs. This value is also updated during the
deletion of a node by the function called fixNodeData(RedBlackNodeString, int).
...
O(1) time.
In the program, I am checking for the case where a particular RedBlackNodeString has
a pointer pointing to nil, since this operation is very common, I have a
function called isNil(RedBlackNodeString), which returns a boolean value of whether
...
Duplicate RedBlackNodeStrings are thought of as being slightly greater than its
...
import Logic.RedBlackString.RedBlackNodeString;

import java.util.ArrayList;
import java.util.List;

// Class Definitions
public class RedBlackTreeString<T extends Comparable<T>> {

    // Root initialized to nil.
    private RedBlackNodeString<T> nil = new RedBlackNodeString<T>();//llama al constructor sin argumentos
    private RedBlackNodeString<T> root = nil;
    ...
    private void leftRotate(RedBlackNodeString<T> x){
    ...
    }// end leftRotateFixup(RedBlackNodeString x)
    ...
    }// end rightRotate(RedBlackNodeString y)


    // @param: y, the node around which the righRotate is to be performed.
    // Updates the numLeft and numRight values affected by the rotate
    private void rightRotateFixup(RedBlackNodeString y){
    ...
    }// end rightRotateFixup(RedBlackNodeString y)


    public void insert(T key, String value) {
        insert(new RedBlackNodeString<T>(key,value));
    }

    // @param: z, the node to be inserted into the Tree rooted at root
    // Inserts z into the appropriate position in the RedBlackTreeString while
    // updating numLeft and numRight values.
    private void insert(RedBlackNodeString<T> z) {

        // Create a reference to root & initialize a node to nil
        RedBlackNodeString<T> y = nil;
        RedBlackNodeString<T> x = root;
        ...
        z.left = nil;
        z.right = nil;
        z.color = RedBlackNodeString.RED;

        // Call insertFixup(z)
        insertFixup(z);

    }// end insert(RedBlackNodeString z)


    // @param: z, the node which was inserted and may have caused a violation
    // of the RedBlackTreeString properties
    // Fixes up the violation of the RedBlackTreeString properties that may have
    // been caused during insert(z)
    private void insertFixup(RedBlackNodeString<T> z){

        RedBlackNodeString<T> y = nil;
        // While there is a violation of the RedBlackTreeString properties..
        while (z.parent.color == RedBlackNodeString.RED){
            ...
            if (z.parent == z.parent.parent.left){
                ...
                if (y.color == RedBlackNodeString.RED){
                    z.parent.color = RedBlackNodeString.BLACK;
                    y.color = RedBlackNodeString.BLACK;
                    z.parent.parent.color = RedBlackNodeString.RED;
                    z = z.parent.parent;
                }
        ...
        root.color = RedBlackNodeString.BLACK;

    }// end insertFixup(RedBlackNodeString z)

    // @param: node, a RedBlackNodeString
    // @param: node, the node with the smallest key rooted at node
    public RedBlackNodeString<T> treeMinimum(RedBlackNodeString<T> node){

        // while there is a smaller key, keep going left
        while (!isNil(node.left))
            node = node.left;
        return node;
    }// end treeMinimum(RedBlackNodeString node)



    // @param: x, a RedBlackNodeString whose successor we must find
    // @return: return's the node the with the next largest key
    // from x.key
    public RedBlackNodeString<T> treeSuccessor(RedBlackNodeString<T> x){
        ...
        RedBlackNodeString<T> y = x.parent;
        ...
        // Return successor
        return y;
    }// end treeMinimum(RedBlackNodeString x)


    // @param: z, the RedBlackNodeString which is to be removed from the the tree
    // Remove's z from the RedBlackTreeString rooted at root
    public void remove(RedBlackNodeString<T> v){

        RedBlackNodeString<T> z = search(v.key);

        // Declare variables
        RedBlackNodeString<T> x = nil;
        RedBlackNodeString<T> y = nil;
        ...
        // RedBlackTreeString properties so call removeFixup()
        if (y.color == RedBlackNodeString.BLACK)
            removeFixup(x);
    }// end remove(RedBlackNodeString z)


    // @param: y, the RedBlackNodeString which was actually deleted from the tree
    // @param: key, the value of the key that used to be in y
    private void fixNodeData(RedBlackNodeString<T> x, RedBlackNodeString<T> y){

        // Initialize two variables which will help us traverse the tree
        RedBlackNodeString<T> current = nil;
        RedBlackNodeString<T> track = nil;
    ...
    // @param: x, the child of the deleted node from remove(RedBlackNodeString v)
    // Restores the Red Black properties that may have been violated during
    // the removal of a node in remove(RedBlackNodeString v)
    private void removeFixup(RedBlackNodeString<T> x){

        RedBlackNodeString<T> w;

        // While we haven't fixed the tree completely...
        while (x != root && x.color == RedBlackNodeString.BLACK){
            ...
            if (x == x.parent.left){
                ...
                if (w.color == RedBlackNodeString.RED){
                    w.color = RedBlackNodeString.BLACK;
                    x.parent.color = RedBlackNodeString.RED;
                    leftRotate(x.parent);
                    w = x.parent.right;
                }

                // Case 2, both of w's children are black
                if (w.left.color == RedBlackNodeString.BLACK &&
                        w.right.color == RedBlackNodeString.BLACK){
                    w.color = RedBlackNodeString.RED;
                    x = x.parent;
                }
                // Case 3 / Case 4
                else{
                    // Case 3, w's right child is black
                    if (w.right.color == RedBlackNodeString.BLACK){
                        w.left.color = RedBlackNodeString.BLACK;
                        w.color = RedBlackNodeString.RED;
                        rightRotate(w);
                        w = x.parent.right;
                    }
                    ...
                    x.parent.color = RedBlackNodeString.BLACK;
                    w.right.color = RedBlackNodeString.BLACK;
                    leftRotate(x.parent);
                    x = root;
                }
        ...
        // Logic.RedBlack tree Properties
        x.color = RedBlackNodeString.BLACK;
    }// end removeFixup(RedBlackNodeString x)
    ...
    public RedBlackNodeString<T> search(T key){

        // Initialize a pointer to the root to traverse the tree
        RedBlackNodeString<T> current = root;
    ...
    public int findNumGreater(RedBlackNodeString<T> node, T key){
    ...
    }// end findNumGreater(RedBlackNodeString, int key)
    ...
    private void getGreaterThan(RedBlackNodeString<T> node, T key,
                                List<T> list) {
        if (isNil(node)) {
            return;
    ...
    public int findNumSmaller(RedBlackNodeString<T> node, T key){
    ...
    }// end findNumSmaller(RedBlackNodeString nod, int key)


    // @param: node, the RedBlackNodeString we must check to see whether it's nil
    // @return: return's true of node is nil and false otherwise
    private boolean isNil(RedBlackNodeString node){

        // return appropriate value
        return node == nil;

    }// end isNil(RedBlackNodeString node)


    // @return: return's the size of the tree
    // Return's the # of nodes including the root which the RedBlackTreeString
    // rooted at root has.
    public int size(){
...
}// end class RedBlackTreeString


/*
Design Decisions:
-----------------
I chose the object RedBlackNodeString class to have seven instance variables which are
all declared public as per the assignment specifications.  Each instance of a
RedBlackNodeString holds a Comparable "key", which is the key of the RedBlackNodeString.  It
...
Each instance also holds a RedBlackNodeString pointer to the node's "parent", "left"
...
also in case we want to just create a RedBlackNodeString and initialize its key later.
...
in the RedBlackTreeString class as it is most referenced there, in this class
